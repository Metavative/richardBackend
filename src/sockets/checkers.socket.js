...
function emitStateToSocket(socket, matchId, st) {
  socket.emit("checkers:state", {
    ...
    state: {
      ...
      playerOneCaptured: st.playerOneCaptured || [],
      playerTwoCaptured: st.playerTwoCaptured || [],
    ...
    socket.on("checkers:move", async (payload) => {
      try {
        ...
        const applied = checkersStore.applyMove(matchId, {
          ...
          isChainContinuation: payload?.isChainContinuation,
          ts: payload?.ts,
        });
        ...
        io.to(room(matchId)).emit("checkers:move_applied", {
          ...
          state: {
            ...
            playerOneCaptured: next.playerOneCaptured || [],
            playerTwoCaptured: next.playerTwoCaptured || [],
